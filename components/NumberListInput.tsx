...
export const NumberListInput = (props: NumberInputProps) => {
	const {
		onChange,
		schemaType,
		value,
	...
	return (
		<Grid columns={range.length} gap={2}>
			{range.map((index) => (
				<Button
					key={index}
...
function generateRange(validation: any[]) {
	const [min, max] = validation.reduce((acc, {_rules}) => {
		return [...acc, ..._rules]
	}, []).filter((rule: any) => ["max", "min"].includes(rule.flag)).map((rule: any) => rule.constraint);
	let range = [];
